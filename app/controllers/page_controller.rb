class PageController < ApplicationController
	...
	def get_statistics
		...
		@statistics[:bids_total_cost] = @bids.map(&:total_cost).inject(&:+) || 0
		@statistics[:verbal_closed_bid_follow_ups_count] = @verbal_closed_bid_follow_ups.count
		@statistics[:verbal_closed_bid_follow_ups_cost] = @verbal_closed_bid_follow_ups.map(&:bid).map(&:total_cost).inject(&:+) || 0
		...
		@statistics[:signed_contracts_cost] = @signed_contracts.map(&:bid).map(&:total_cost).inject(&:+) || 0
		@statistics[:outgoing_communications] = @bids_follow_ups.count
		@statistics[:scheduled_appointments] = @inspections.count
		@statistics[:inspection_gross_income] = @inspections.map(&:appointment).map(&:inspFee).inject(&:+) || 0
